...
Wire = Union[int, PlainText]  # Wire number
...
class Connector(TopLevelGraphicalComponent):
    ...
    label: Optional[bool] = None  # set False to suppress this connector's auto-derived label box
...
class Cable(TopLevelGraphicalComponent):
    ...
    # twisted pairs/groups within this cable, by 1-based wire index, e.g. [[1, 2], [3, 4]].
    # Pairs/groups are always listed explicitly (no boolean shorthand) for full control.
    twisted: List[List[int]] = field(default_factory=list)
    ...
    def __post_init__(self) -> None:
        ...
        # normalize & validate twisted pairs/groups (1-based wire ids within this cable)
        seen_wires = set()
        normalized_twisted = []
        for group in self.twisted:
            if not isinstance(group, (list, tuple)) or len(group) < 2:
                raise Exception(
                    f"Cable {self.designator}: each twisted group must list at least "
                    f"two wire ids, got {group!r}."
                )
            if len(set(group)) != len(group):
                raise Exception(
                    f"Cable {self.designator}: a wire is repeated within a twisted "
                    f"group ({group!r})."
                )
            for wid in group:
                if (
                    ...
                    or not (1 <= wid <= self.wirecount)
                ):
                    raise Exception(
                        f"Cable {self.designator}: twisted wire id {wid!r} is not an "
                        f"integer in 1..{self.wirecount}."
                    )
                if wid in seen_wires:
                    raise Exception(
                        f"Cable {self.designator}: wire {wid} appears in more than one "
                        f"twisted group."
                    )
                seen_wires.add(wid)
            normalized_twisted.append([int(wid) for wid in group])
        self.twisted = normalized_twisted
